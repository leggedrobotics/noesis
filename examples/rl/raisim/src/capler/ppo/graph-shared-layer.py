    ...
    with graph.as_default():
        with tf.device("/device:CPU:0"):
            ...
            with tf.name_scope('Example/Agent'):
                ...
                common_units = [64]
                common_activations = [tf.nn.tanh]
                common_net = ns.nn.MultiLayerPerceptron(inputs=obs, units=common_units, activations=common_activations)

                # Define actor network
                policy_units = [64]
                policy_activations = [tf.nn.tanh]
                policy_net = ns.nn.MultiLayerPerceptron(inputs=[common_net.output], units=policy_units, activations=policy_activations)
                # Create a policy
                ...
                critic_units = [64]
                critic_activations = [tf.nn.tanh]
                critic_net = ns.nn.MultiLayerPerceptron(inputs=[common_net.output], units=critic_units, activations=critic_activations)
                # Create a critic
