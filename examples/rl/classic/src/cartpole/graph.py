...
def main():
    ...
    obs_specs = [
        tf.TensorSpec(name="motion", shape=[4], dtype=tf.float32),
    ]

    # Discrete actions
    act_specs = [tf.TensorSpec(name="force", shape=[1], dtype=tf.float32)]
    act_categories = [["push_left", "push_right"]]
    ...
    with graph.as_default():
        with tf.device("/device:CPU:0"):
            ...
            with tf.name_scope('Example/Agent'):
                ...
                # VERSION 2: Categorical policy
                # TODO: COMMENT IN THE CODE BELOW
                # policy = ns.func.CategoricalPolicy(
                #     latent=policy_net,
                #     actions_specs=act_specs,
                #     actions_categories=act_categories,
                #     name="Policy",
                # )
