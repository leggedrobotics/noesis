 ...
 */
#ifndef NOESIS_GYM_ENVS_MUJOCO_HALFCHEETAH_HALFCHEETAH_ENVIRONMENT_HPP_
#define NOESIS_GYM_ENVS_MUJOCO_HALFCHEETAH_HALFCHEETAH_ENVIRONMENT_HPP_
...
#include "noesis/gym/envs/mujoco/halfcheetah/HalfcheetahObservations.hpp"
#include "noesis/gym/envs/mujoco/halfcheetah/HalfcheetahActions.hpp"
 ...
 * Reference: https://github.com/openai/gym/blob/master/gym/envs/mujoco/half_cheetah_v3.py
 ...
 */
class HalfcheetahEnvironment final : public noesis::gym::Environment<float>
{
public:
  // Aliases
  ...
  explicit HalfcheetahEnvironment(
    size_t control_decimation = 5,
    double discount_factor = 0.995,
    double reset_noise_factor = 0.1,
    bool enable_logging = false,
    bool visualize = false,
    const std::string &name = "Halfcheetah",
    const std::string &scope = "/",
    bool verbose = false
  ) :
    Base(
      HalfcheetahObservations<Scalar>().spec(),
      HalfcheetahActions<Scalar>().spec(),
      {"total", "foward_velocity", "control_cost"},
      name,
      scope,
      verbose
    ),
  ...
  {
    NWARNING_IF(isVerbose(), "[HalfcheetahEnv]: New instance at: " << std::hex << this);
    // Simulation
    MujocoSimulationConfig config;
    // Set the default MJCF file to load
    std::string modelFile = mujoco::assets() + "/benchmarks/half_cheetah.xml";
  ...
  ~HalfcheetahEnvironment() final = default;
...
private:
  ...
  inline void extract_commands(const Actions &actions)
  {
    ...
    using Index = HalfcheetahActions<Scalar>::Index;
  ...
  inline void compute_rewards(Rewards &rewards)
  {
    ...
    VectorX weights = VectorX::Ones(3);
    // Set reward weights
    weights(1) = 1.0; // forward velocity
    weights(2) = 0.1; // control cost
  ...
  inline Termination check_for_termination()
  {
    ...
    // No terimation conditions
    termination = {static_cast<Scalar>(0), Termination::Type::Unterminated};
  ...
  inline void observations_from_kinematics(Observations &observations)
  {
    ...
    using Index = HalfcheetahObservations<Scalar>::Index;
...
#endif // NOESIS_GYM_ENVS_MUJOCO_HALFCHEETAH_HALFCHEETAH_ENVIRONMENT_HPP_
