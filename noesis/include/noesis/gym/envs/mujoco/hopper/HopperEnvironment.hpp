 ...
 */
#ifndef NOESIS_GYM_ENVS_MUJOCO_HOPPER_HOPPER_ENVIRONMENT_HPP_
#define NOESIS_GYM_ENVS_MUJOCO_HOPPER_HOPPER_ENVIRONMENT_HPP_
...
#include "noesis/gym/envs/mujoco/hopper/HopperObservations.hpp"
#include "noesis/gym/envs/mujoco/hopper/HopperActions.hpp"

namespace noesis {
namespace gym {

/*!
 * @brief Implementation of the mujoco hopper environment.
 * Goal: Make a two-dimensional one-legged robot hop forward as fast as possible.
 * Reference: https://github.com/openai/gym/blob/master/gym/envs/mujoco/hopper_v3.py
 ...
 * Observation: Generalized coordinates (except for absolute X position), Generalized velocities
 * Action: control signals for the actuators
 */
class HopperEnvironment final : public noesis::gym::Environment<float>
{
public:
  // Aliases
  ...
  static constexpr size_t JointsDim = 3;
  ...
  explicit HopperEnvironment(
    size_t control_decimation = 4,
    double discount_factor = 0.995,
    double reset_noise_factor = 5e-3,
    bool enable_logging = false,
    bool visualize = false,
    const std::string &name = "Hopper",
    const std::string &scope = "/",
    bool verbose = false
  ) :
    Base(
      HopperObservations<Scalar>().spec(),
      HopperActions<Scalar>().spec(),
      {"benchmark"},
      name,
      scope,
      verbose
  ...
  {
    NWARNING_IF(isVerbose(), "[HopperEnv]: New instance at: " << std::hex << this);
    // Simulation
    MujocoSimulationConfig config;
    // Set the default MJCF file to load
    std::string modelFile = mujoco::assets() + "/benchmarks/hopper.xml";
  ...
  ~HopperEnvironment() final = default;
...
private:
  ...
  inline void extract_commands(const Actions &actions)
  {
    ...
    using Index = HopperActions<Scalar>::Index;
  ...
  inline Termination check_for_termination()
  {
    ...
    // TC1: base state is outside range
    for (size_t j = 0; j < BasePoseDim + JointsDim; ++j) {
      if (q(j) < -100 || q(j) > 100)
        termination = {static_cast<Scalar>(0), Termination::Type::TerminalState};
    }
    for (size_t j = 0; j < BaseVelocityDim + JointsDim; ++j) {
      if (u(j) < -100 || u(j) > 100)
        termination = {static_cast<Scalar>(0), Termination::Type::TerminalState};
    }
    // TC2: base height falls below certain range
    if (q(1) <= 0.7) {
      termination = {static_cast<Scalar>(0), Termination::Type::TerminalState};
    }
    // TC3: Legs cross over each other
    if (q(2) <= -0.2 || q(2) >= 0.2) {
      termination = {static_cast<Scalar>(0), Termination::Type::TerminalState};
    }
  ...
  inline void observations_from_kinematics(Observations &observations)
  {
    ...
    using Index = HopperObservations<Scalar>::Index;
...
#endif // NOESIS_GYM_ENVS_MUJOCO_HOPPER_HOPPER_ENVIRONMENT_HPP_
