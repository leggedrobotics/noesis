 ...
 */
#ifndef NOESIS_RL_ALGORITHM_POLICY_EVALUATION_DAMPED_POLICY_EVALUATION_HPP_
#define NOESIS_RL_ALGORITHM_POLICY_EVALUATION_DAMPED_POLICY_EVALUATION_HPP_
 ...
 * with linear interpolation of the target values according to [1].
 * It is used for training state-value function approximators.
 *
...
class DampedPolicyEvaluation final: public ::noesis::core::Object
{
public:
  ...
  DampedPolicyEvaluation() = delete;
  
  DampedPolicyEvaluation(DampedPolicyEvaluation&& other) = default;
  DampedPolicyEvaluation& operator=(DampedPolicyEvaluation&& other) = default;
  
  DampedPolicyEvaluation(const DampedPolicyEvaluation& other) = delete;
  DampedPolicyEvaluation& operator=(const DampedPolicyEvaluation& other) = delete;
  
  explicit DampedPolicyEvaluation(
      ValueFunction* critic,
      const std::string& scope,
      const std::string& name):
    Base(name, scope),
    damping_(0.1, utils::make_namescope({scope, name, "damping"}), {0.0, 1.0}),
    ...
    valueFunction_(critic)
  {
    ...
    hyperparam::manager->addParameter(damping_);
  ...
  ~DampedPolicyEvaluation() override {
    // Remove hyper-parameters from the global manager
    hyperparam::manager->removeParameter(damping_);
  ...
  void configure() {
    ...
    NINFO("[" << ns << "]: Damping: " << static_cast<Scalar>(damping_));
    ...
    trainOutputsBuffer_.emplace_back("loss/value_prediction/squared/loss_op");
    ...
    trainOutputsBuffer_.emplace_back("damping/value");
    ...
    metrics_.clear();
    metrics_.push_back("DPE/value_error_loss");
    metrics_.push_back("DPE/l2_regularization");
    metrics_.push_back("DPE/total_loss");
    metrics_.push_back("DPE/gradient_norm");
    metrics_.push_back("DPE/clipped_gradient_norm");
    metrics_.push_back("DPE/learning_rate");
    metrics_.push_back("DPE/damping");
  }
  
  void initialize()  {
    const auto ns = this->namescope();
    NFATAL_IF(!graph_->isActive(), "[" << ns << "]: Failed to initialize: Graph session is not active!");
    graph_->writeModifiable<Scalar>(ns + "/configurations/damping", damping_);
  ...
  void train(
    ...
    metrics_["DPE/value_error_loss"] = trainOutputsBuffer_[0][0];
    metrics_["DPE/l2_regularization"] = trainOutputsBuffer_[1][0];
    metrics_["DPE/total_loss"] = trainOutputsBuffer_[2][0];
    metrics_["DPE/gradient_norm"] = trainOutputsBuffer_[3][0];
    metrics_["DPE/clipped_gradient_norm"] = trainOutputsBuffer_[4][0];
    metrics_["DPE/learning_rate"] = trainOutputsBuffer_[5][0];
    metrics_["DPE/damping"] = trainOutputsBuffer_[6][0];
  }
...
private:
  ...
  hyperparam::HyperParameter<Scalar> damping_;
...
#endif // NOESIS_RL_ALGORITHM_POLICY_EVALUATION_DAMPED_POLICY_EVALUATION_HPP_
