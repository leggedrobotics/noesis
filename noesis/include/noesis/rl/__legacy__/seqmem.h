 ...
 */
#ifndef NOESIS_MEMORY_SEQUENCE_VECTOR_MEMORY_HPP_
#define NOESIS_MEMORY_SEQUENCE_VECTOR_MEMORY_HPP_
...
public:
  ...
  void insertRight(size_t instance, const std::vector<MatrixType>& inputs) {
    NFATAL_IF(instance >= numberOfInstances_,
              "[" << getScopedName() << "]: 'instance' (" << instance
                  << ") must be less than the number of instances (" << numberOfInstances_ << ")!");
    NFATAL_IF(inputs.size() != sequences_.size(),
    ...
    // Shift all current elements by one time-step and add the new values at the front
    for (size_t k = 0; k < inputs.size(); ++k) {
      buffers_[instance][k] = sequences_[k](instance).asEigenMatrix().rightCols(sequenceSize_ - 1); //why use the buffer ?
      sequences_[k](instance).asEigenMatrix().leftCols(sequenceSize_ - 1) = buffers_[instance][k];
      sequences_[k](instance).asEigenMatrix().rightCols(1) = inputs[k];
    }
  }
  ...
  void extract(size_t instance, std::vector<MatrixType>& outputs) {
    ...
    NFATAL_IF(outputs.size() != sequences_.size(),
              "[" << getScopedName() << "]: 'outputs' does not have the same size as the histories: "
  ...
  void extractFromRight(size_t instance, std::vector<MatrixType>& outputs) {
    NFATAL_IF(instance >= numberOfInstances_,
              "[" << getScopedName() << "]: 'instance' (" << instance
                  << ") must be less than the number of instances (" << numberOfInstances_ << ")!");
    NFATAL_IF(outputs.size() != sequences_.size(),
...
#endif // NOESIS_MEMORY_SEQUENCE_VECTOR_MEMORY_HPP_
