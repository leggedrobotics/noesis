 ...
 */
#ifndef NOESIS_RL_SAMPLE_TRAJECTORY_SAMPLER_HPP_
#define NOESIS_RL_SAMPLE_TRAJECTORY_SAMPLER_HPP_
...
public:
  ...
  TrajectorySampler() = delete;
  
  //! @note Move and move-assignment construction is permissible for this class.
  TrajectorySampler(TrajectorySampler&& other) noexcept = default;
  TrajectorySampler& operator=(TrajectorySampler&& other) noexcept = default;
  
  //! @note Copy and copy-assignment construction is permissible for this class.
  TrajectorySampler(const TrajectorySampler& other) = default;
  TrajectorySampler& operator=(const TrajectorySampler& other) = default;
  ...
  ~TrajectorySampler() final = default;
  ...
  bool sample() override {
    // Step 1: Generate  action samples given observations
    // from the previous transition
    agent_->explore(environment_->observations(), environment_->actions());
    // Step 2: Generate transition samples given the
    // actions generated by the agent.
    environment_->step();
    // Step 3: After samples have been generated in the environment's `step()` call,
    ...
    // Step 4: Since transitions were added above, we can now check if the target
    // agent's criteria for terminating sampling are met.
    bool isReady = agent_->ready();
    // Step 5: We proceed to perform episode termination checks for this round transitions.
    // TODO: (@vt): can this also be parallelized?
    for (size_t e = 0; e < Ne; e++) {
      // Retrieve instance-specific references
      auto& env = (*environment_)[e];
      ...
      if (static_cast<int>(trm.type) >= 0) {
        // Step 6: We terminate trajectories when they time-out or a terminal state
        ...
        if ( trm.type != Termination::Type::Unterminated || isReady ) {
          ...
          NWARNING_IF(isReady && isVerbose(), "[" << namescope() << "]: Environment terminated:"
            ...
            << "\n  at Instance: " << e)
          // Step 7: Start a new episode only if the agent has not signalled that it has
          // enough data to perform training. Otherwise deactivate the environment instance.
          if (!isReady) {
            environment_->reset(e);
            memory_->initializeTrajectory(e, obs);
            episodeTerminalMetrics_[e].push_back(Vector::Zero(environment_->metrics().size()));
            episodeStepMetrics_[e].push_back(Vector::Zero(environment_->metrics().size()));
            episodeRewards_[e].push_back(Vector::Zero(environment_->tasks().size()));
    ...
    // Step 8: Signal whether sampling operations have completed.
    // NOTE: (1) Completion is defined according to the the agent's criteria for experience accrual.
    // NOTE: (2) This implementation terminates all active episodes as soon this condition is met.
    return isReady;
  }
...
#endif // NOESIS_RL_SAMPLE_TRAJECTORY_SAMPLER_HPP_
