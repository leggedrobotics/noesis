...
using TD(1) and damped value-error objective.
"""
...
from noesis.core.loss import squared


class DampedPolicyEvaluation(tf.Module):
    """Implements the Damped Policy Evaluation algorithm based on [1].
    ...
    def __init__(self,
                 ...
                 l2r_weight=0.0,
                 max_grad_norm=1.0):
        super(DampedPolicyEvaluation, self).__init__(name=name)
        ...
        with tf.name_scope(self.name_scope.name):
            ...
            with tf.name_scope("configurations"):
                ...
                damping, _ = modifiable(damping, dtype=dtype, name="damping")
            ...
            # Wrap the configured initial value with a decay operation
            damping = tf1.train.exponential_decay(
                damping,
                global_step=local_step,
                decay_rate=dp_decay_rate,
                decay_steps=dp_decay_steps,
                name="decays/damping"
            )
            damping = tf.identity(damping, name="damping/value")
            ...
            with tf.name_scope("loss"):
                with tf.name_scope("value_prediction"):
                    value_target = damping * value_target_in + (1 - damping) * value_previous_in
                    value_error_loss_op = squared(target=value_target, value=value_predicted, name='loss_op')
